...
class Pasien_pemeriksaan_input extends CI_Controller
{
	...
	{
		$id_kunjungan = $this->input->post('pasien_kunjungan_id');
		$id_pasien = $this->db->query("SELECT pasien.id as id from pasien_kunjungan join pasien on pasien.id = pasien_kunjungan.pasien_id where pasien_kunjungan.id = $id_kunjungan")->row()->id;
