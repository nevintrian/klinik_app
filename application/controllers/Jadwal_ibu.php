...
class Jadwal_ibu extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->session->userdata('level') == 'kader') {
            ...
            $jadwal_ibu = $this->m_jadwal_ibu->get_limit_data_bidan();
        } else {
    ...
    public function cetak_pdf($id)
    {
        ...
        if (is_numeric($id)) {
            ...
            $pdf->Cell(0, 7, "DATA JADWAL IBU HAMIL POSYANDU $nama", 0, 1, 'C');
        } else {
        ...
        $pdf->Cell(30, 6, 'Nama Kegiatan', 1, 0, 'C');
        $pdf->Cell(60, 6, 'Nama Imunisasi', 1, 0, 'C');
        $pdf->Cell(75, 6, 'Nama Penyuluhan', 1, 0, 'C');
        ...
        if(is_numeric($id) && $this->session->userdata('level') == 'bidan'){
            ...
            $barang = $this->db->query("SELECT jadwal_ibu.*, kegiatan.nama as kegiatan_nama, penyuluhan_ibu.nama as penyuluhan_ibu_nama, imunisasi_ibu.nama as imunisasi_ibu_nama, posyandu.nama as posyandu_nama FROM jadwal_ibu join kegiatan on kegiatan.id = jadwal_ibu.kegiatan_id join penyuluhan_ibu on penyuluhan_ibu.id = jadwal_ibu.penyuluhan_ibu_id join imunisasi_ibu on imunisasi_ibu.id = jadwal_ibu.imunisasi_ibu_id join posyandu on jadwal_ibu.posyandu_id = posyandu.id join bidan_posyandu on posyandu.id = bidan_posyandu.posyandu_id join bidan on bidan_posyandu.bidan_id = bidan.id where bidan.id = $bidan_id")->result();
        }
        $no = 1;
        foreach ($barang as $data) {
            $pdf->Cell(8, 6, $no, 1, 0);
            $pdf->Cell(45, 6, $data->jadwal, 1, 0);
            $pdf->Cell(30, 6, $data->kegiatan_nama, 1, 0);
            $pdf->Cell(60, 6, $data->imunisasi_ibu_nama, 1, 0);
            $pdf->Cell(75, 6, $data->penyuluhan_ibu_nama, 1, 0);
