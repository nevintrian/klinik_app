...
class Jadwal_balita extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->session->userdata('level') == 'kader') {
            ...
            $jadwal_balita = $this->m_jadwal_balita->get_limit_data_bidan();
        }else {
    ...
    function send_whatsapp()
    {
        ...
        foreach ($telepon_unique as $telepon) {
		    ...
		    "Authorization: Bearer EAAYIhixngCoBAB505i65jR10TTQTVYMKY52ZCizFnHavNrrUXFx5rZBrXpOZAi7hNfIvaEoH6xkbfScoSxBdhMLmiPq3vMsXzq3g9xVPgRJrPakTjuP3UBjGSZAmLRZB4UFCyBef8H2jdSO547xdqfjayJZBm0jYTzEZC4XD8mNqv0AbOe94avv",
    ...
    public function cetak_pdf($id)
    {
        ...
        if (is_numeric($id)) {
            ...
            $pdf->Cell(0, 7, "DATA JADWAL BALITA POSYANDU $nama", 0, 1, 'C');
        } else {
        ...
        $pdf->Cell(60, 6, 'Nama Imunisasi', 1, 0, 'C');
        $pdf->Cell(60, 6, 'Nama Penyuluhan', 1, 0, 'C');
        $pdf->Cell(60, 6, 'Nama Posyandu', 1, 1, 'C');
        ...
        if(is_numeric($id) && $this->session->userdata('level') == 'bidan'){
            ...
            $barang = $this->db->query("SELECT jadwal_balita.*, kegiatan.nama as kegiatan_nama, penyuluhan_balita.nama as penyuluhan_balita_nama, imunisasi_balita.nama as imunisasi_balita_nama, posyandu.nama as posyandu_nama FROM jadwal_balita join kegiatan on kegiatan.id = jadwal_balita.kegiatan_id join penyuluhan_balita on penyuluhan_balita.id = jadwal_balita.penyuluhan_balita_id join imunisasi_balita on imunisasi_balita.id = jadwal_balita.imunisasi_balita_id join posyandu on jadwal_balita.posyandu_id = posyandu.id join bidan_posyandu on posyandu.id = bidan_posyandu.posyandu_id join bidan on bidan_posyandu.bidan_id = bidan.id where bidan.id = $bidan_id")->result();
        }
        $no = 1;
        foreach ($barang as $data) {
            ...
            $pdf->Cell(60, 6, $data->imunisasi_balita_nama, 1, 0);
            $pdf->Cell(60, 6, $data->penyuluhan_balita_nama, 1, 0);
            $pdf->Cell(60, 6, $data->posyandu_nama, 1, 1);
            $no++;
        }
